...
static volatile float32_t samplesMain[OVER_FRAME_LEN*2];
...
{
    return (FRAME_LEN/2)/(float32_t)nr_bins(FRAME_LEN);
}
...
void adc_isr(void) 
{
	...
	samplesMain[i++] = convert_adc_u12_sample_to_s16(adc_read_regular(ADC1)&ADC_DR_DATA_MASK);
	...
	if (i%OVER_FRAME_LEN == 0) {
		full_samples_frame = samplesMain+(i-OVER_FRAME_LEN);
